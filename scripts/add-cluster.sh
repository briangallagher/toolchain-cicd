...
login_to_cluster() {
    if [[ ${SINGLE_CLUSTER} != "true" ]]; then
      if [[ -z ${KUBECONFIG} ]] && [[ -z ${SANDBOX_CONFIG} ]]; then
        ...
        # REGISTER_SERVER_API=$(/Users/briangallagher/go/bin/yq4.16.1 -r .\"$1\".serverAPI ${SANDBOX_CONFIG})
        # REGISTER_SA_TOKEN=$(/Users/briangallagher/go/bin/yq4.16.1 -r .\"$1\".tokens.registerCluster ${SANDBOX_CONFIG})
        REGISTER_SERVER_API=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"$1\".serverAPI ${SANDBOX_CONFIG})
        REGISTER_SA_TOKEN=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"$1\".tokens.registerCluster ${SANDBOX_CONFIG})        
...
if [[ -n ${SANDBOX_CONFIG} ]]; then
    OPERATOR_NS=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"${JOINING_CLUSTER_TYPE}\".sandboxNamespace ${SANDBOX_CONFIG})
    CLUSTER_JOIN_TO_OPERATOR_NS=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"${TARGET_CLUSTER_NAME}\".sandboxNamespace ${SANDBOX_CONFIG})
...
if [[ ${LETS_ENCRYPT} == "true" ]]; then
    SA_CA_CRT=`curl https://letsencrypt.org/certs/lets-encrypt-r3.pem | base64 -i -`
else
    SA_CA_CRT=`oc get secret ${SA_SECRET} -n ${OPERATOR_NS} -o json ${OC_ADDITIONAL_PARAMS} | jq -r '.data["ca.crt"]'`
fi

if [[ -n ${SANDBOX_CONFIG} ]]; then
    API_ENDPOINT=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"${JOINING_CLUSTER_TYPE}\".serverAPI ${SANDBOX_CONFIG})
    JOINING_CLUSTER_NAME=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"${JOINING_CLUSTER_TYPE}\".serverName ${SANDBOX_CONFIG})

    login_to_cluster ${CLUSTER_JOIN_TO}

    CLUSTER_JOIN_TO_NAME=$(/Users/briangallagher/go/bin/yq4.16.1 e .\"${CLUSTER_JOIN_TO}\".serverName ${SANDBOX_CONFIG})
else
